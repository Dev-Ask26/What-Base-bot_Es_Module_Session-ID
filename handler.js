    ...
    for (const file of files) { 
        if (file.endsWith(".js")) { 
            try { 
                ...
                if (cmd?.name && typeof cmd.run === "function") commands.set(cmd.name, cmd.run); 
            } catch (err) { 
...
async function handler(socket, m, msg, rawMsg) { 
    try { 
        ...
        const isSudo = config.SUDO?.some((num) => sender?.includes(num)) || false;
        const botNumber = socket.user.id.split(":")[0] + "@s.whatsapp.net";
        const chatId = m.key.remoteJid;
        const groupMetadata = chatId.endsWith("@g.us") ? await socket.groupMetadata(chatId) : null;
